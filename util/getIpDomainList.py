...
class GetIpDomainList():
    def __init__(self, ipDomainInputList):
        self.projectName = ""
        self.ipDomainInputList = ipDomainInputList
        self.ipList = []
        self.domainList = []
    ...
    def start(self):
        # get ip, domain input , and projectName
        self.splitIpDomain(self.ipDomainInputList)
    ...
    def splitIpDomain(self, ipDomainInputList):
        ...
        ipDomain = list(ipDomainInputList)                                     # 用户的输入可能有连续的空格, split()会获取到空字符串, 所以之后要用filter把空字符串去掉, 返回值非list, 用list()转换
